...
\begin{table}[h]
  \caption{Source-side results for ERM, DAN-DG and SAM, with the two additional
  $\lambda_{\mathrm{DG}}$ settings of the controlled study below the rule. Each cell gives
  accuracy / macro-F1 (\%) on the held-out validation split of that source domain. Worst is
  the lowest per-domain value of each metric; the weakest domain is Art Painting for every
  model except DAN-DG ($\lambda_{\mathrm{DG}}=10$), where it is Cartoon. These are the only
  quantities available for model selection, and checkpoints were selected on mean macro-F1.}
...
\begin{table}[h]
  \caption{Sketch results and the two source-side diagnostics. Sketch accuracy and macro-F1
  are measured once on the complete Sketch domain after every Task 3 configuration was
  frozen; $\Delta$ columns are changes relative to ERM in percentage points. Sep.\ is the
  held-out accuracy of a multinomial logistic-regression probe ($C=1$, 70/30 split, seed
  6304) predicting Photo, Art Painting or Cartoon from frozen source-validation features;
  chance is 33.3\%. The sharpness proxy is measured on a fixed batch of 32 validation images
  per source domain (96 in total): $\mathcal L$ is the validation cross-entropy,
  $\mathcal L_{+\epsilon}$ the cross-entropy after one normalised gradient-ascent step of
  radius 0.05, and $\Delta_{\mathrm{sharp}}$ their difference.}
...
\begin{figure}[t]
  ...
  \caption{Mean-source, worst-source and Sketch macro-F1 for every Task 3 model, including
  the two additional $\lambda_{\mathrm{DG}}$ settings. The Sketch value is annotated above
  each orange bar. Source bars are close together for the four stable models, while their
  Sketch values span 55.9 to 72.6.}
  \label{fig:task3_method_comparison}
\end{figure}

\begin{table}[t]
  \caption{Per-class Sketch accuracy (\%) with the change against ERM in percentage points
  in brackets. Support is the number of Sketch images in each class. The final column is
  the target-aware DAN model from Task 2, measured against the same ERM checkpoint.}
...
Good performance on the source domain does not guarantee good performance on the target domain, and this is visible via the results as well. A high mean source accuracy / macro-F1 is observed in all 3 methods, where the ERM baseline mean source accuracy / macro-F1 is 94.49 / 94.51; the worst-performing source domain scored 92.64 points. Similarly, the mean source accuracy / macro-F1 for DAN-DG (with $\lambda=1.0$) and SAM (with $\rho=0.05$) is 91.82 / 91.56 and 95.16 / 95.12, respectively. Their worst-performing source macro-F1 is 89.35 and 92.34, respectively. While establishing a possible link between the validation numbers of source domains and the test numbers of the target domains, ERM, as anticipated, performed worst in the target domain, whereas DAN-DG has the lowest mean source accuracy of 91.82 and best target accuracy/macro-F1 of 68.44/72.37. SAM has performed the highest mean source accuracy of 95.16, but still its target accuracy is 3.89 points lower than DAN-DG. This shows there is no linear relation between the source and target domain performances. Therefore, it is difficult to establish that source validation performance reliably predicts performance on Sketch.

The details hidden by the aggregated source metrics come to light when looking at the class-level failures. ERM performs very poorly on Sketch for giraffe, dog, horse, and house, with accuracies of 23.51\%, 36.79\%, 45.59\%, and 45.00\%, whereas in the case of DAN-DG, 5 out of 7 classes gained points (house by 48.75 points, horse by 27.45, giraffe by 21.38, and guitar by 12.50). However, for the person, the performance declined by 33.75 points. Therefore, high aggregate source performance hides substantial target-specific class failures, particularly the degradation of person and the continued difficulty of dog recognition. These results are recorded in \texttt{per\_class\_sketch.csv}.
...
\begin{figure}[t]
  ...
  \caption{Training curves for all Task 3 models; ERM is the reused Task 2 run. (a)
  Classification loss on the source batches, on a logarithmic axis. (b) The weighted
  pairwise source MMD penalty $\lambda_{\mathrm{DG}}\cdot\mathrm{MMD}^2$ for the three
  DAN-DG settings. (c) Mean source-validation macro-F1, the checkpoint-selection signal.
  At $\lambda_{\mathrm{DG}}=10$ the classification loss stays between 1.74 and 1.95 for
  all 23 epochs, close to the $\ln 7 = 1.946$ of a uniform seven-class prediction, and
  source-validation macro-F1 never exceeds 16.06\%. Curves end at the epoch where early
  stopping triggered; the selected checkpoints are epoch 11 (ERM), 1 (DAN-DG,
  $\lambda_{\mathrm{DG}}=1$), 11 (SAM), 7 ($\lambda_{\mathrm{DG}}=0.1$) and 18
  ($\lambda_{\mathrm{DG}}=10$).}
...
\begin{figure}[t]
  ...
  \caption{The two source-side diagnostics plotted against Sketch macro-F1. (a)
  Source-domain separability; the dotted line marks chance (33.3\%). ERM and SAM share the
  same separability (86.38\%) but differ by 11.74 points of Sketch macro-F1. (b) The
  sharpness proxy $\Delta_{\mathrm{sharp}}$; lower means a smaller loss increase under the
  fixed perturbation of radius 0.05. SAM has the lowest value, but DAN-DG
  ($\lambda_{\mathrm{DG}}=1$) and DAN-DG ($\lambda_{\mathrm{DG}}=0.1$) reach higher Sketch
  macro-F1 with larger values.}
  \label{fig:task3_diagnostics}
\end{figure}

A lower common sharpness proxy under given perturbation partially supports the claim about local stability, but it does not establish that the overall loss surface is flatter and will generalize better on any domain shifts. The model trained with SAM perturbation did appear to be a little less sensitive to any minor change in its weights. SAM's loss increased by just 0.141, as compared to ERM and DAN-DG, which reported a change of 0.600 and 0.631, respectively. Therefore, the local stability ranking is
...
Both methods improve substantially over the shared ERM baseline:

\begin{table}[h]
  \caption{Target-aware DAN (Task 2) and target-free DAN-DG (Task 3) against the shared ERM
  checkpoint. Both use the same MMD implementation and RBF kernel construction.}
...
\begin{figure}[t]
  ...
  \caption{Per-class Sketch accuracy change against the shared ERM checkpoint for
  target-aware DAN (Task 2) and target-free DAN-DG (Task 3). Both improve
  \texttt{giraffe}, \texttt{guitar}, \texttt{horse} and \texttt{house}. They diverge on
  \texttt{dog} ($+26.81$ for DAN, $-0.13$ for DAN-DG) and \texttt{elephant} ($-1.62$
  against $+7.43$), and both lose accuracy on \texttt{person}, DAN-DG by far more ($-33.75$
  against $-1.25$).}
